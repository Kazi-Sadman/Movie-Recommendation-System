...
async def lifespan(app: FastAPI):
    ...
    # Load pickles on application startup
    ...
    # Cleanup logic (if needed on shutdown) can go here
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Suitable for local streamlit / dev environments
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    """Safe TMDB GET request handling."""
    q = dict(params)
...
async def search_bundle(
    ...
    # Concurrent fetch of TMDB cards for recommendations
    rec_titles = [title for title, _ in recs]
    cards = await asyncio.gather(*[attach_tmdb_card_by_title(t) for t in rec_titles])
